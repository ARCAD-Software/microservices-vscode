...
> [!NOTE]  
> This feature is limited to management actions related to ARCAD Transformer Microservices functionalities. Additionally, some operations may experience minor delays.

## Web Server Management

The Web server management in ARCAD Transformer Microservices uses **QShell commands** from the IBM i Integrated Web Services server.  
...
|---|---|
| Server Name | Sets the name of the Server. This parameter is mandatory. The name must be unique, less than 10 characters, must contain at least one alpha character, cannot contain a blank, and cannot contain the special characters `& ! @ % * : / \\ # ; ? , = ^ < > \| + ' " ( ) { } [ ] -`. |
| Server Description | Sets a succint description of the Server. This parameter is optional. |
| Installation Directory | Sets the path to the Installation directory. Note that it must be an existing IFS directory. You can browse and copy the path from the IFS Browser (**View > IBM i IFS Browser**). |
| Starting Port | Sets a port number. The expected value is a string between 1 and 65535. |
...
| HTTPS Secure Port | Sets a port number. The expected value is a string between 1 and 65535. |
| Disable non-SSL port after TLS configuration | Once TLS is configured, this parameter disables plain HTTP access on the non-secure port. |
| IBMi User Profile | Sets an IBM i user profile with `*ALLOBJ` and `*IOSYSCFG` special authority is required. |
...
Installation Directory: `/arcad`  
Starting Port: `12344`  
Enable SSL/TLS checked, HTTPS Secure Port: `34567`  
IBMi User Profile = `AKUMAR`.
...
> [!WARNING]  
> Deleted web servers cannot be recovered.  
